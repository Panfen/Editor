...
export default () => {
  ...
  const insertText = (text) => {
    ...
    if (anchorNode.parentNode.className === 'tag') {
      ...
      const { content } = docData.nodes[curParIndex].nodes[curSegIndex];
  ...
  const breakParagraph = () => {
    ...
    const { content } = docData.nodes[parIndex].nodes[segIndex];
  ...
  /**
	 *
	 */
  const getIndex = (data, id) => data.nodes.findIndex(item => item.id === id);
